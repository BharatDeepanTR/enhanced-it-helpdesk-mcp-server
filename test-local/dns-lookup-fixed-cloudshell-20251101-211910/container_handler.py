#!/usr/bin/env python3
"""
Container handler for DNS lookup service.
Adapts the lambda function for containerized execution with HTTP interface.
"""
...
# Import the main lambda function
try:
    from chatops_route_dns_intent import lambda_handler, lookup_dns_record
...
class DNSLookupHandler(BaseHTTPRequestHandler):
    """HTTP request handler for DNS lookup service"""
    ...
    def do_POST(self):
        ...
        if self.path == '/lookup' or self.path == '/':
            self.handle_post_lookup()
        else:
            self.send_error(404, "Not Found")
...
class DNSLookupServer:
    ...
    def start(self):
        ...
        try:
            self.server = HTTPServer((self.host, self.port), DNSLookupHandler)
