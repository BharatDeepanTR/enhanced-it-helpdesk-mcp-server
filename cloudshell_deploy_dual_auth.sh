...
echo "🚀 Deploying Dual-Auth Application Details MCP Client to CloudShell"
...
'''
    ...
    print(f"✅ Created dual_auth_app_details_client.py ({len(client_code)} bytes)")
    return True

def run_test():
    """Run the dual authentication test"""
    print("\n🧪 Running dual authentication test...")
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"⚠️ Test completed with warnings (code {result.returncode})")
...
def main():
    print("🚀 CloudShell Deployment: Dual-Auth Application Details MCP Client")
    ...
    if not create_dual_auth_client():
        print("❌ Deployment failed - could not create client")
        return False
    
    print("✅ Deployment successful!")
    print("📁 Files created:")
    print("   • dual_auth_app_details_client.py - Main client")
    print()
...
echo "📁 Created CloudShell deployment script: /tmp/deploy_dual_auth_app_details.py"
echo ""
echo "🚀 To deploy and test:"
echo "   python3 /tmp/deploy_dual_auth_app_details.py"
echo ""
echo "📋 This script will:"
echo "   ✅ Install dependencies (requests-aws4auth, boto3)"
echo "   ✅ Create dual-authentication client"
echo "   ✅ Run comprehensive test using calculator's patterns"
echo "   ✅ Test both Agent API and HTTP SigV4 methods"
echo ""
echo "🔑 Key Differences from Previous Approach:"
echo "   • Uses Bedrock Agent Runtime API (calculator's primary method)"
echo "   • Falls back to HTTP with SigV4 (calculator's secondary method)"
echo "   • Same authentication patterns that worked for calculator"
